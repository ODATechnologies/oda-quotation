import { fmtNumber } from "./helpers";

export function exportToPdf(data) {
  const { docNo, staff, supplier, customer, contact, items, terms, totalSupply, totalVat, grandTotal } = data;

  const logoUrl  = `${window.location.origin}${import.meta.env.BASE_URL}logo.png`;
  const shortNo  = docNo.replace("ODA-", "");
  const fileName = `Quotation for ${customer} ${shortNo}`;
  const ODA_COLOR = "#F84F04";   // R:248 G:79 B:4
  const DARK      = "#1a1a1a";

  const html = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<title>${fileName}</title>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Noto Sans KR','Arial',sans-serif;font-size:10.5px;color:#111;background:#fff;padding:12mm 12mm 10mm;}
@page{size:A4;margin:0;}
@media print{body{padding:8mm 10mm;}}

/* ── 상단 헤더: 로고 좌 / QUOTATION 우 ── */
.top-header{
  display:flex;
  align-items:flex-end;
  justify-content:space-between;
  margin-bottom:0;
  padding-bottom:6px;
}
.logo-wrap img{height:48px;object-fit:contain;display:block;}
.title-block{text-align:right;line-height:1;}
.quotation-title{
  font-size:30px;font-weight:700;letter-spacing:4px;
  color:#111;display:block;
}
.subtitle{
  font-size:7.5px;letter-spacing:1.8px;
  color:#888;margin-top:4px;display:block;
}

/* ── 오렌지 구분선 ── */
.orange-bar{height:3px;background:${ODA_COLOR};margin-bottom:9px;}

/* ── INFO 2컬럼 ── */
.info-wrap{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:9px;}
.info-box{border:1px solid #ccc;overflow:hidden;}
.info-title{
  background:${DARK};color:#fff;
  font-size:8.5px;font-weight:700;letter-spacing:.1em;
  padding:4px 9px;
}
.info-row{display:flex;border-top:1px solid #e2e2e2;}
.info-label{
  background:#f4f4f4;font-size:8.5px;font-weight:700;color:#555;
  padding:4px 8px;min-width:82px;width:82px;
  border-right:1px solid #e2e2e2;
  display:flex;align-items:center;
}
.info-value{
  padding:4px 8px;font-size:9px;
  display:flex;align-items:center;flex:1;
}
.info-value.accent{color:${ODA_COLOR};font-weight:600;}

/* ── 품목 테이블 ── */
.item-table{width:100%;border-collapse:collapse;font-size:9.5px;}
.item-table thead tr{background:${ODA_COLOR};}
.item-table th{
  color:#fff;padding:6px 5px;text-align:center;
  font-size:8.5px;font-weight:700;letter-spacing:.04em;
  border-right:1px solid rgba(255,255,255,.25);white-space:nowrap;
}
.item-table th:last-child{border-right:none;}
.item-table td{
  padding:5px 6px;
  border-bottom:1px solid #ebebeb;
  border-right:1px solid #f0f0f0;
  vertical-align:middle;
}
.item-table td:last-child{border-right:none;}
.item-table tr:nth-child(even) td{background:#fafafa;}
.detail-row td{
  background:#f5f7fb!important;color:#666;
  font-size:8.5px;padding:2px 6px 2px 18px;
  border-bottom:1px solid #f0f0f0;
}
td.c{text-align:center;}td.r{text-align:right;}td.b{font-weight:700;}

/* ── 하단 ── */
.bottom-section{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:9px;}
.terms-box{border:1px solid #ccc;overflow:hidden;}
.terms-title{
  background:${DARK};color:#fff;
  font-size:8.5px;font-weight:700;letter-spacing:.07em;
  padding:4px 9px;
}
.terms-row{display:flex;border-top:1px solid #e2e2e2;}
.terms-label{
  background:#f4f4f4;font-size:8.5px;font-weight:700;color:#555;
  padding:4px 8px;min-width:90px;width:90px;
  border-right:1px solid #e2e2e2;display:flex;align-items:center;
}
.terms-value{padding:4px 8px;font-size:9px;flex:1;display:flex;align-items:center;}
.amt-box{border:1px solid #ccc;overflow:hidden;}
.amt-row{
  display:flex;justify-content:space-between;align-items:center;
  padding:5px 10px;border-bottom:1px solid #e2e2e2;font-size:9.5px;
}
.amt-row:last-child{border-bottom:none;}
.amt-row.total{
  background:${ODA_COLOR};color:#fff;
  font-weight:700;font-size:11.5px;padding:7px 10px;
}
.footer{
  margin-top:10px;text-align:center;color:#aaa;
  font-size:8px;border-top:1px solid #eee;padding-top:6px;
}
</style>
</head>
<body>

<!-- 상단: 로고 좌 / QUOTATION 우 (같은 행, 하단 정렬) -->
<div class="top-header">
  <div class="logo-wrap">
    <img src="${logoUrl}" alt="ODA Technologies" onerror="this.style.display='none'"/>
  </div>
  <div class="title-block">
    <span class="quotation-title">QUOTATION</span>
    <span class="subtitle">PARTNER SHIP, CORE POWER OF ODA</span>
  </div>
</div>
<div class="orange-bar"></div>

<!-- INFO -->
<div class="info-wrap">
  <div class="info-box">
    <div class="info-title">CUSTOMER</div>
    <div class="info-row"><div class="info-label">Bill To</div><div class="info-value b">${customer||''}</div></div>
    <div class="info-row"><div class="info-label">Attention</div><div class="info-value">${contact.name||''}</div></div>
    <div class="info-row"><div class="info-label">Phone</div><div class="info-value">${contact.phone||''}</div></div>
    <div class="info-row"><div class="info-label">E-mail</div><div class="info-value">${contact.email||''}</div></div>
  </div>
  <div class="info-box">
    <div class="info-title">ODA TECHNOLOGIES</div>
    <div class="info-row"><div class="info-label">Doc. No.</div><div class="info-value accent">${docNo}</div></div>
    <div class="info-row"><div class="info-label">Supplier</div><div class="info-value accent">${supplier.name}</div></div>
    <div class="info-row"><div class="info-label">Reg. No.</div><div class="info-value accent">${supplier.bizNo}</div></div>
    <div class="info-row"><div class="info-label">Sales Rep.</div><div class="info-value">${staff.name||''}</div></div>
    <div class="info-row"><div class="info-label">Phone</div><div class="info-value">${staff.phone||''}</div></div>
  </div>
</div>

<!-- 품목 -->
<table class="item-table">
  <thead>
    <tr>
      <th style="width:22px">NO</th>
      <th style="width:22%">Description</th>
      <th style="width:15%">Model</th>
      <th style="width:36px">Qty</th>
      <th style="width:13%">Unit Price</th>
      <th style="width:13%">Amount</th>
      <th style="width:10%">VAT</th>
      <th>Remark</th>
    </tr>
  </thead>
  <tbody>
    ${items.map((item,idx)=>`
    <tr>
      <td class="c">${idx+1}</td>
      <td class="b">${item.category||''}</td>
      <td>${item.spec||''}</td>
      <td class="c">${item.qty}</td>
      <td class="r">${fmtNumber(item.unitPrice)}</td>
      <td class="r b">${fmtNumber(item.amount)}</td>
      <td class="r">${fmtNumber(item.vat)}</td>
      <td>${item.note||''}</td>
    </tr>
    ${(item.details||[]).map(d=>`<tr class="detail-row"><td></td><td colspan="7">- ${d}</td></tr>`).join('')}
    `).join('')}
  </tbody>
</table>

<!-- 하단 -->
<div class="bottom-section">
  <div class="terms-box">
    <div class="terms-title">TERMS &amp; CONDITIONS</div>
    <div class="terms-row"><div class="terms-label">Delivery</div><div class="terms-value">${terms.delivery||''}</div></div>
    <div class="terms-row"><div class="terms-label">Validity</div><div class="terms-value">${terms.validity||''}</div></div>
    <div class="terms-row"><div class="terms-label">Payment</div><div class="terms-value">${terms.payment||''}</div></div>
  </div>
  <div class="amt-box">
    <div class="amt-row"><span>Supply Amount</span><span>₩ ${fmtNumber(totalSupply)}</span></div>
    <div class="amt-row"><span>VAT (10%)</span><span>₩ ${fmtNumber(totalVat)}</span></div>
    <div class="amt-row total"><span>TOTAL</span><span>₩ ${fmtNumber(grandTotal)}</span></div>
  </div>
</div>

<div class="footer">
  We hereby submit our quotation as above. &nbsp;|&nbsp;
  Date: ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})} &nbsp;|&nbsp;
  ODA Technologies Co., Ltd.
</div>

<script>window.onload=function(){window.print();window.onafterprint=function(){window.close();};};<\/script>
</body></html>`;

  const w = window.open('','_blank','width=960,height=760');
  w.document.write(html);
  w.document.close();
}

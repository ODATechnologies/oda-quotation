...
export default function QuoteDashboard({ showToast }) {
  ...
  return (
    <div>
      ...
      {/* 국내 요약 */}
      <div style={{ fontSize:12, color:"var(--text-muted)", fontWeight:500, textTransform:"uppercase", letterSpacing:".06em", marginBottom:8 }}>국내 견적</div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:14 }}>
        {[
          ...
          <div key={label} style={{ background:"var(--surface-1)", borderRadius:8, padding:"14px 16px" }}>
            <div style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:6 }}>{label}</div>
            <div style={{ fontSize:20, fontWeight:500, color:"#185FA5" }}>{value}</div>
            <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:3 }}>이번 {period==="yearly"?"연도":period==="monthly"?"달":"날"}</div>
          </div>
        ))}
      </div>

      {/* 해외 요약 */}
      <div style={{ fontSize:12, color:"var(--text-muted)", fontWeight:500, textTransform:"uppercase", letterSpacing:".06em", marginBottom:8 }}>해외 견적</div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:20 }}>
        {[
          ...
          <div key={label} style={{ background:"var(--surface-1)", borderRadius:8, padding:"14px 16px" }}>
            <div style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:6 }}>{label}</div>
            <div style={{ fontSize:20, fontWeight:500, color:"#0F6E56" }}>{value}</div>
            <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:3 }}>이번 {period==="yearly"?"연도":period==="monthly"?"달":"날"}</div>
          </div>
        ))}
      </div>
      ...
      {/* 필터 바 */}
      <div style={{ background:"var(--surface-1)", borderRadius:8, padding:"14px 16px", marginBottom:16, display:"flex", gap:10, alignItems:"flex-end", flexWrap:"wrap" }}>
        {[
        ...
        return (
          <div key={key} style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
              ...
              <span style={{ fontSize:12, color:"var(--text-muted)" }}>{quotes.length}건</span>
              {domAmt > 0 && <span style={{ fontSize:12, color:"#185FA5" }}>국내 ₩{fmtNumber(domAmt)}</span>}
              {ovsAmt > 0 && <span style={{ fontSize:12, color:"#0F6E56" }}>해외 {fmtUSD(ovsAmt)}</span>}
            </div>
